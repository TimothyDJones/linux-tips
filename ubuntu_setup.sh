...
curl -o /tmp/CascadiaCode-2102.03.zip -J -L https://downloads.sourceforge.net/cascadia-code.mirror/CascadiaCode-2102.03.zip
cd /tmp
dtrx -n /tmp/CascadiaCode-2102.03.zip
sudo cp -R /tmp/CascadiaCode-2102.03/ttf/* /usr/local/share/fonts
...
if [[ "${DISTRIB_CODENAME:0:2}" =~ ^(gr)$ ]]; then     # Groovy (20.10)
	APP_VERSION=0.99.30-6_amd64
elif [[ "${DISTRIB_CODENAME:0:2}" =~ ^(fo)$ ]]; then     # Focal (20.04)
    APP_VERSION=0.99.30-5_amd64
else
...
APP_VERSION=0.8.6
APP_EXT=deb
FILE_NAME=${APP_NAME,,}_${APP_VERSION}_Ubuntu_20.10_amd64
